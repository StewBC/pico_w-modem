...
void ultoa(unsigned long value, char *string, int radix)
{
    ...
    do
    {
        ...
        if (buffer[index] > '9')
            buffer[index] += 'A' - ':'; /* continue with A, B,... */
        value /= radix;
    } while (value != 0);
...
String::String(const char *cstr, int len)
{
    init();
    if (-1 == len)
        len = strlen(cstr);
...
String::String(float value, unsigned char decimalPlaces)
{
    init();
    char buf[33];
    sprintf(buf, "%*.*f", value, (decimalPlaces + 2), decimalPlaces);
    *this = buf;
...
String::String(double value, unsigned char decimalPlaces)
{
    init();
    char buf[33];
    sprintf(buf, "%*.*lf", value, (decimalPlaces + 2), decimalPlaces);
    *this = buf;
    // *this = dtostrf(value, (decimalPlaces + 2), decimalPlaces, buf);
}
...
unsigned char String::concat(float num)
{
    char buf[20];
    int len;
    len = sprintf(buf, "%4.2f", num);
    return concat(buf, len);
